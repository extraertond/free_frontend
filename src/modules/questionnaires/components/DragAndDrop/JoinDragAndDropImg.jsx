...
const JoinDragAndDropImg = ({ username, currentQuestionnaire, setCurrentQuestionnaire }) => {
  useEffect(() => {
              ...
              <div className="row">
                        ...
                        {currentQuestion?.options.map((e, index) => (
                          <Draggable key={e.id} draggableId={e.id.toString()} index={index}>
                            {(draggableProvided) => (
                              ...
                              >
                                <img src={e.image_url} className="img-fluid rounded img-item" alt="help_image" />
                              </li>
                            )}
                          </Draggable>
                        ))}
                ...
                <div className="col" style={{ textAlign: "center" }}>
                  {currentQuestion?.targetCategories.map((category, index) => {
                    return (
                      <Droppable droppableId={category.id.toString()} key={index} direction="horizontal">
                        {(droppableProvided) => (
                          <ul
                            {...droppableProvided.droppableProps}
                            ref={droppableProvided.innerRef}
                            className="task-container-join-img"
                          >
                            <div
                              style={{
                                fontSize: "15px",
